...
const ListProducts = ({productData,navigation}) => {
  ...
  const RenderItem = ({item}) => (
    ...
    <View style={styles.viewItemName}>
      <Text style={styles.nameItem}>{ item.name.length > 30 ? item.name.substring(0, 40) + '...' : item.name}</Text>
    </View>
    ...
    <View style={styles.viewItemOldPrice}>
      <Text>{item.oldPrice = parseFloat(item.price)*(item.discount/100) }</Text>
  ...
  return (
      <View style={{flex: 1,height:12000}}>
        ...
        <View style={styles.groupWorkspaces}>
          ...
          <View style={{ flexDirection: 'row' }}>
            ...
            >
              <Text style={activeTab === 'Tab 3' ? styles.isTextTitle : styles.textTitle}>Bestseller - Ngoại Văn</Text>
              {
                 activeTab === 'Tab 3' ? <View style={styles.lineTitle}></View>:null
              }
          ...
          <View style={styles.groupItem}>
            ...
            {activeTab === 'Tab 1' ? (
              <FlashList
                data={productData.filter((item) => item.categoryId.name === "TrendByDay")}
...
<FlashList
  data={productData.filter((item) => item.categoryId.name === "Bestseller")}
  renderItem={({ item }) => <RenderItem item={item} />}
  horizontal
  initialNumToRender={3}
  estimatedItemSize={15}
